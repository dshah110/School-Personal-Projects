...
void walk_el(int update_time, int time_between, int verb) {
    ...
    for (el = g_el->next; el != g_el; el = el->next) {
        ...
        //routing table updated at this point

        //do DVA and send out to ppl. After wards, we listen and recieve.

        //setting up poll so can recieve updates and run DVA.
        struct pollfd sockets[256] = {0};
        int sc = 0; int md = -1;
        struct link *myLs = get_ls()->next;
        while (myLs->name != NULL) {
            if (get_myid() == myLs->peer0) {
                sockets[sc].fd = myLs->sockfd0;
                sockets[sc].events = POLLIN;
            } else if (get_myid() == myLs->peer1) {
                sockets[sc].fd = myLs->sockfd1;
                sockets[sc].events = POLLIN;
            }
            if (sockets[sc].fd > md) {
                md = sockets[sc].fd;
        ...
        //want to send our rt tbale to the other nodes rn.
        ...
        for(;;) {
            int ud = 0;
            int itt = 0;
            if(poll(sockets, sc, 0) > 0) {
                for(int i = 0; i < sc; i++) {
                    if(sockets[i].revents && POLLIN) {
                        struct sockaddr_storage clntAddr;
                        socklen_t clntAddrLen = sizeof(clntAddr);
                        char rcvd[1032];
                        recvfrom(sockets[i].fd, rcvd, 1032, 0, (struct sockaddr *) &clntAddr, &clntAddrLen);

                        struct link *tempLs = get_ls()->next;//g_ls->next;
                        int sender;
                        while (tempLs != get_ls()) {//g_ls) {
                            if (tempLs->sockfd0 == sockets[i].fd) {
                                sender = tempLs->peer1;
                            } else if (tempLs->sockfd1 == sockets[i].fd) {
                                sender = tempLs->peer0;
                            }
                            tempLs = tempLs->next;
                        }
                        ...
                        //BREAK OUT OF THIS LOOP WHEN TIME EXCEEDED UPDATE TIME.
                    }
                }
            }
        }
