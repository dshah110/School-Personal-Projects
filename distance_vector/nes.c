...
int send_rt(uint8_t *buffer){
	...
	for (temp = g_rt->next; temp != g_rt; temp = temp->next){
...
void walk_el(int update_time, int time_between, int verb)
{
	...
	for (el = g_el->next ; el != g_el ; el = el->next) {
		...
		for (struct rte *i = g_rt->next; i != g_rt; i= i->next){
			update_flag = false;
			node me = get_myid();
			node dest = i->d;
			node nh = i->nh;
		...
		for (struct link *i = g_ls->next; i != g_ls; i= i->next){
			if (i->sockfd0 != -1){
...
void dispatch_event(struct es* es)
{
	...
	switch (es->ev) {
		case _es_link:
			...
			if(es->peer0 == (int) get_myid()) {
				rt = find_rte(es->peer1);
				if(rt != NULL) {
					if(update_rte(es->peer1, es->cost, es->peer0) < 0) {
						if(add_rte(es->peer1,es->cost,es->peer0) < 0) {
							perror("add or update failure");
						}
					}
				}
			}
			else {
				rt = find_rte(es->peer0);
				if(rt != NULL) {
					if((int) rt->c > es->cost) {
						update_rte(es->peer0,es->cost,es->peer1); //error
					}
				}
				else{ 
		...
		case _ud_link:
			...
			if(l != NULL) {
				if(l->peer0 == get_myid()) {
					update_rte(l->peer1, es->cost, l->peer0);
				}
				else {
					update_rte(l->peer0, es->cost, l->peer1);
				}
			}
			break;
		case _td_link:
			...
			if(l != NULL) {
				if(l->peer0 == get_myid()) {
					update_rte(l->peer1, -1, l->peer0);
				}
				else {
					update_rte(l->peer0,-1,l->peer1);
				}
			}
			break;
		default:
...
// void getLinkName(node src, node dest, char* string) {
// 	//char* source;
// 	//char* destination;
// 	string = malloc(9);
// 	string[0] = 'L';
// 	if(src < dest) {
// 		memcpy(string[1], &src, 4);
// 		memcpy(string[5], &dest, 4);
// 	}
// 	else {
// 		memcpy(string[1], &dest, 4);
// 		memcpy(string[5], &src, 4);
// 	}
// }
...
// 			dispatch_event(es);
// 		}
// 		struct rte *i = get_rt_head();
// 		for (i->next; i != get_rt_head(); i= i->next){
// 			node current = get_myid();
// 			node dest = i->d;
// 			node next_hop = i->nh;
// 			char* dstring[8];
// 			char* nhstring[8];

// 			getLinkName(current, dest, dstring);
// 			getLinkName(current, next_hop, nhstring);

// 			struct link *destLink = find_link(dstring);
// 			struct link *nhlink = find_link(nhstring);

// 			if(dest == next_hop) {
// 				if(nhlink == 0x0) { //link DNE
// 					update_rte(dest, -1, dest);
// 				}
// 				else {	//link exists
// 					update_rte(dest, find_link(nhstring)->c, dest); //update rte with cost of nh link
// 				}
// 			}
// 			else {
// 				if(destLink == 0x0) {
...
// 					}
// 					else {
// 						update_rte(dest, i->c + nhlink->c, next_hop); // minus old cost
// 					}
// 				}
// 				else {
// 					if(nhlink != 0x0) {
// 						update_rte(dest, destLink->c, dest);
// 					}
// 					else {
// 						if(destLink->c < i->c + nhlink->c) {
// 							update_rte(dest, destLink->c, next_hop);
// 						}
// 						else {
// 							update_rte(dest, i->c + nhlink->c, next_hop); //may have to minus old cost
// 						}
// 					}
// 				}
// 			}

// 			uint32_t oldCosts[255];
...
// 		}
// 		//want to execute events in an event set sequentially here
